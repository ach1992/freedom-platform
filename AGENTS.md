...
Default the PR boundary to the **meaningful outcome**, not to an implementation layer or seam. When one Task/FAST outcome remains safely reviewable as one candidate, prefer one PR and one acceptance/review cycle. Split an outcome across partial PRs only when dependency/integration sequencing, materially different risk or rollback/release boundaries, distinct review ownership, platform constraints, or effective-diff reviewability genuinely require it. Do not create separate PRs merely for schema/service/handler/presentation/test/localization/report-operation seams that belong to the same accepted outcome. Conversely, a Phase-level cumulative PR is exceptional: use it only when the current residual itself is cohesive and reviewable, never to hide unrelated outcomes in one candidate.
...
High/Critical financial, authorization, security, provider, schema, deployment/release, secret, or irreversible work requires independent review and explicit Owner approval before merge unless that exact action was explicitly pre-authorized. Once a candidate is frozen after focused validation and self-review, exact-head CI and independent review should normally proceed **in parallel** because they are independent evidence streams; integration still waits for both to pass on the same current candidate/target envelope. The applicable CI tier must pass on the final candidate; merge style never substitutes for review or validation.
...
For this repository, required independent review must run in a reviewer context independent from the authoring Master. Prefer a directly dispatchable fresh independent reviewer capability when one is actually available and can receive the exact review envelope without sharing the authoring context; otherwise give the Owner one ready-to-paste prompt for a fresh ChatGPT chat. A platform-native reviewer object is required only when repository/platform protection requires it. Do not substitute a non-independent self-review merely to avoid the relay. The canonical review contract is in `CONTRIBUTING.md`.
