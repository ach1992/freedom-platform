...
trait AgentPricingPersistenceSupport
{
    ...
    {
        $value = $definition->snapshot();
        $value['profile_code'] = $profileCode;
    ...
    private function verifyRuleVersion(object $row, string $profileCode): void
    {
        ...
        if ($row->action !== null && $action === null) {
            throw new RuntimeException('Stored rule action is invalid.');
        }
    ...
    private function json(array $value): string
    {
        ...
        if (strlen($json) > 8192) {
            throw new RuntimeException('Agent pricing snapshot exceeds the storage boundary.');
        }
    ...
    private function jsonObject(string $value): array
    {
        ...
        if (! is_array($decoded) || array_is_list($decoded)) {
            throw new RuntimeException('Stored agent pricing snapshot is invalid.');
        }
    ...
    private function mutationKey(string $key): void
    {
        if (preg_match('/\A[A-Za-z0-9:_.-]{8,128}\z/', $key) !== 1) {
            throw new DomainException('Agent pricing mutation key is invalid.');
        }
    }
