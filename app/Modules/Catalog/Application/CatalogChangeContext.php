...
final readonly class CatalogChangeContext
{
    public function __construct(
        ...
        if (preg_match('/\A[A-Za-z0-9:_-]{16,128}\z/', $requestFingerprint) !== 1) {
            throw new InvalidArgumentException('Catalog request fingerprint is invalid.');
        }

        if (preg_match('/\A[A-Za-z0-9:_-]{16,64}\z/', $correlationId) !== 1) {
            throw new InvalidArgumentException('Catalog correlation ID is invalid.');
        }

        if (preg_match('/\A[a-z0-9_.-]{1,64}\z/', $reasonCode) !== 1) {
            throw new InvalidArgumentException('Catalog reason code is invalid.');
        }

        if ($reason !== null && (mb_strlen($reason) > 1000 || preg_match('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $reason) === 1)) {
            throw new InvalidArgumentException('Catalog reason is invalid.');
        }

        if ($actorAdministratorId < 1) {
            throw new InvalidArgumentException('Catalog administrator ID must be positive.');
        }
    }
