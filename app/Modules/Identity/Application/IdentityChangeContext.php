...
final readonly class IdentityChangeContext
{
    public function __construct(
        ...
        if (preg_match('/\A[A-Za-z0-9:_-]{16,128}\z/', $requestFingerprint) !== 1) {
            throw new InvalidArgumentException('Identity request fingerprint is invalid.');
        }
        if (preg_match('/\A[A-Za-z0-9:_-]{16,64}\z/', $correlationId) !== 1) {
            throw new InvalidArgumentException('Identity correlation ID is invalid.');
        }
        if (preg_match('/\A[a-z0-9_.-]{1,64}\z/', $reasonCode) !== 1) {
            throw new InvalidArgumentException('Identity reason code is invalid.');
        }
        if ($reason !== null
            && (mb_strlen($reason) > 1000
                || preg_match('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $reason) === 1)) {
            throw new InvalidArgumentException('Identity reason is invalid.');
        }
        if ($actorAdministratorId !== null && $actorAdministratorId < 1) {
            throw new InvalidArgumentException('Identity administrator ID must be positive.');
        }
        if ($actorUserId !== null && $actorUserId < 1) {
            throw new InvalidArgumentException('Identity user ID must be positive.');
        }
        if (($actorAdministratorId === null) === ($actorUserId === null)) {
            throw new InvalidArgumentException('Exactly one identity mutation actor is required.');
        }
    }
    ...
    public function requireReason(): string
    {
        ...
        if ($reason === '') {
            throw new InvalidArgumentException('An explanatory identity reason is required.');
        }
