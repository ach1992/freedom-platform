...
final readonly class LocalizationChangeContext
{
    public function __construct(
        ...
        if (preg_match('/\A[A-Za-z0-9:_-]{16,128}\z/', $requestFingerprint) !== 1) {
            throw new InvalidArgumentException('Localization request fingerprint is invalid.');
        }
        if (preg_match('/\A[A-Za-z0-9:_-]{16,64}\z/', $correlationId) !== 1) {
            throw new InvalidArgumentException('Localization correlation ID is invalid.');
        }
        if (preg_match('/\A[a-z0-9_.-]{1,64}\z/', $reasonCode) !== 1) {
            throw new InvalidArgumentException('Localization reason code is invalid.');
        }
        if ($reason !== null && (mb_strlen($reason) > 1000 || preg_match('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $reason) === 1)) {
            throw new InvalidArgumentException('Localization reason is invalid.');
        }
        if ($actorAdministratorId < 1) {
            throw new InvalidArgumentException('Localization administrator ID must be positive.');
        }
    }
