...
trait PaymentEligibilityRuleSelection
{
    ...
    private function selectRule(
        ...
        usort($qualified, function (array $left, array $right): int {
            $scoreComparison = $this->compareScore($right['score'], $left['score']);
            if ($scoreComparison !== 0) {
                return $scoreComparison;
            }

            return strcmp($left['row']->rule_code, $right['row']->rule_code);
        });

        $bestScore = $qualified[0]['score'];
        $best = array_values(array_filter(
            $qualified,
            fn (array $candidate): bool => $this->compareScore($candidate['score'], $bestScore) === 0,
        ));
        $effects = array_values(array_unique(array_map(
            static fn (array $candidate): string => $candidate['row']->effect,
            $best,
        )));
        if (count($effects) > 1) {
