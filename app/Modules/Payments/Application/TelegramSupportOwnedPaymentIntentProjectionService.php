...
final readonly class TelegramSupportOwnedPaymentIntentProjectionService implements TelegramSupportOwnedPaymentIntentProjection
{
    ...
    public function pageForSelf(int $actorUserId, int $subjectUserId, int $page, int $pageSize): TelegramSupportBusinessReferencePage
    {
        ...
        $totalItems = (int) $connection->table('payment_intents')->where('user_id', $subjectUserId)->count();
        $totalPages = max(1, (int) ceil($totalItems / $pageSize));
        $effectivePage = min($page, $totalPages);

        $rows = $connection->table('payment_intents')
            ->where('user_id', $subjectUserId)
            ->orderByDesc('created_at')
            ->orderByDesc('id')
            ->offset(($effectivePage - 1) * $pageSize)
            ->limit($pageSize)
            ->get(['public_id', 'amount_irr', 'currency']);

        $items = [];
        foreach ($rows as $row) {
            $publicId = $this->databaseString($row->public_id ?? null, 'Payment Intent public ID');
            $items[] = new TelegramSupportBusinessReferenceListItem(
                $this->selectionToken($subjectUserId, $publicId),
                $publicId,
                $this->nonNegativeDatabaseInt($row->amount_irr ?? null, 'Payment Intent amount'),
                $this->databaseString($row->currency ?? null, 'Payment Intent currency'),
            );
        }
    ...
    public function resolveForSelf(int $actorUserId, int $subjectUserId, string $selectionToken): TelegramSupportBusinessReferenceResolution
    {
        ...
        $row = $this->database->connection()->table('payment_intents as intent')
            ->where('intent.user_id', $subjectUserId)
            ->whereRaw("LEFT(SHA2(CONCAT('telegram-support-owned-payment-intent-v1:', CAST(intent.user_id AS CHAR), ':', intent.public_id), 256), 40) = ?", [$selectionToken])
            ->first(['intent.id', 'intent.public_id']);

        if ($row === null) {
            throw new AuthorizationException('Telegram Support Payment Intent reference is unavailable for this actor.');
        }

        return new TelegramSupportBusinessReferenceResolution(
            $this->positiveDatabaseInt($row->id ?? null, 'Payment Intent ID'),
            $this->databaseString($row->public_id ?? null, 'Payment Intent public ID'),
        );
    }
    ...
    private function assertSelf(int $actorUserId, int $subjectUserId): void
    {
        if ($actorUserId < 1 || $subjectUserId < 1 || $actorUserId !== $subjectUserId) {
            throw new AuthorizationException('Telegram Support Payment Intent projection is self-only.');
        }
    }
    ...
    private function assertPage(int $page, int $pageSize): void
    {
        if ($page < 1 || $pageSize < 1 || $pageSize > self::MAXIMUM_PAGE_SIZE) {
            throw new InvalidArgumentException('Telegram Support Payment Intent page request is invalid.');
        }
    }
    ...
    {
        return substr(hash('sha256', "telegram-support-owned-payment-intent-v1:{$userId}:{$publicId}"), 0, 40);
    }
