...
final class ServiceSyncDatabaseAuthority
{
    ...
    {
        self::set($connection, self::LEASE, null, $serviceId, null, $leaseToken);
    }

    public static function snapshot(Connection $connection, int $runId, int $serviceId, string $correlationId): void
    {
        self::set($connection, self::SNAPSHOT, $runId, $serviceId, null, $correlationId);
    }
...
SQL,
                [$authority, $runId, $serviceId, $actorId, $requestHash, $correlationId],
            );
        } catch (Throwable $exception) {
            self::disconnect($connection);
            throw $exception;
