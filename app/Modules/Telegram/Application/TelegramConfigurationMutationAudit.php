...
final readonly class TelegramConfigurationMutationAudit
{
    ...
    public function existing(string $action, string $requestFingerprint, bool $lock = false): ?TelegramConfigurationMutationReceipt
    {
        ...
        if ($row->target_type === null || $row->target_id === null || ! ctype_digit($row->target_id)) {
            throw new RuntimeException('Stored Telegram configuration audit target is invalid.');
        }

        $before = $this->decode($row->before_safe_data);
        $after = $this->decode($row->after_safe_data);

        return new TelegramConfigurationMutationReceipt(
            $action,
            $row->target_type,
            (int) $row->target_id,
            $before,
    ...
    public function record(
        ...
        return new TelegramConfigurationMutationReceipt($action, $targetType, $targetId, $before, $after, $changed);
    }
    ...
    private function decode(?string $encoded): array
    {
        ...
        if (! is_array($decoded)) {
            throw new RuntimeException('Stored Telegram configuration audit data is invalid.');
        }
