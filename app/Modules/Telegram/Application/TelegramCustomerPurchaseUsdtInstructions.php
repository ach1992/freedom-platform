...
final readonly class TelegramCustomerPurchaseUsdtInstructions
{
    public function __construct(
        ...
        foreach ([$authorityPublicId, $paymentIntentPublicId, $amountQuotePublicId, $orderPublicId, $quotePublicId, $decisionPublicId] as $publicId) {
        ...
        if ($network !== 'BEP20'
            ...
            || ! is_numeric($exactUsdt)
            || preg_match('/\A(?:0|[1-9][0-9]*)(?:\.[0-9]{1,6})?\z/', $exactUsdt) !== 1
            || bccomp($exactUsdt, '0', 6) <= 0
            || $expiresAt->getTimestamp() < 1) {
            throw new InvalidArgumentException('Telegram USDT payment instructions are invalid.');
        }
    }
}
