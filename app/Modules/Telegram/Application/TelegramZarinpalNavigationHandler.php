...
final readonly class TelegramZarinpalNavigationHandler
{
    ...
    private function translation(string $key, string $locale, array $replace = []): string
    {
        ...
        if ($value === '' || $value === '['.$key.']') {
            throw new RuntimeException('Telegram Zarinpal translation is unavailable.');
        }
        if (preg_match('/:[A-Za-z_][A-Za-z0-9_]*/', $value) === 1) {
            throw new RuntimeException('Telegram Zarinpal translation has an unresolved placeholder.');
        }
