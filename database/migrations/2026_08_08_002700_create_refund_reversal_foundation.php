...
return new class extends Migration
{
    ...
    private function createRefundabilityGuards(): void
    {
        DB::unprepared(implode("\n", [
            ...
            '      AND posted_debit_irr = expected_total_irr',
            '      AND posted_credit_irr = expected_total_irr',
            '      AND entry_count >= 2;',
            '',
            '    IF eligible_parent_count <> 1 THEN',
            "        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Refundability must be attached atomically before ledger finalization.';",
            '    END IF;',
            'END',
        ]));
