...
return new class extends Migration
{
    ...
    private function addChecks(): void
    {
        ...
        DB::statement("ALTER TABLE payment_method_rule_versions ADD CONSTRAINT payment_rule_time_chk CHECK ((`starts_at_utc` IS NULL AND `ends_at_utc` IS NULL) OR (`starts_at_utc` REGEXP '^[0-2][0-9]:[0-5][0-9]$' AND `ends_at_utc` REGEXP '^[0-2][0-9]:[0-5][0-9]$'))");
        ...
        DB::statement("ALTER TABLE payment_method_eligibility_decision_methods ADD CONSTRAINT payment_decision_method_code_chk CHECK (`method_code` REGEXP '^[a-z][a-z0-9_.-]{1,63}$' AND `method_version` >= 1 AND `route_order` >= 1)");
