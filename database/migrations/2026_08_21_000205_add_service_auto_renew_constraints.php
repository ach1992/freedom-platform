...
return new class extends Migration
{
    ...
    private function addChecks(): void
    {
        foreach ([
            ...
            ['service_auto_renew_attempts', 'sara_retry_chk', "(`state` = 'retry_pending' AND `next_retry_at` IS NOT NULL AND (`retry_count` >= 1 OR (`retry_count` = 0 AND `reason_code` = 'renewal_window_unsafe'))) OR (`state` = 'insufficient_wallet' AND `next_retry_at` IS NOT NULL AND `retry_count` >= 1) OR (`state` NOT IN ('retry_pending','insufficient_wallet') AND `next_retry_at` IS NULL)"],
