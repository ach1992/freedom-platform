...
{
    /** @requirement PAY-001 ACL-001 ACL-002 SEC-002 */
    public function run(): void
    {
        ...
        $permissionId = DB::table('permissions')->where('code', 'payment_providers.manage')->value('id');
        if ((! is_int($roleId) && ! is_string($roleId)) || (! is_int($permissionId) && ! is_string($permissionId))) {
            throw new RuntimeException('Payment eligibility access seed references an unknown finance role or permission.');
        }
