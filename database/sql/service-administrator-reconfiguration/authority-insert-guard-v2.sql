    ...
    IF NEW.authorization_mode = 'paid_purchase' THEN
       ...
       OR NOT EXISTS (
           ...
           WHERE operation_row.id = NEW.provisioning_operation_id
             ...
             AND preview_row.actor_administrator_id IS NULL
             AND preview_row.administrator_reason_code IS NULL
             AND preview_row.administrator_reason IS NULL
        ...
        END IF;
    ELSEIF NEW.authorization_mode IN ('no_charge','administrator_no_charge') THEN
        IF COALESCE(@app_service_mutation_authority, '') <> 'service_reconfiguration_no_charge_queue_v1'
           ...
           OR NEW.result_recorded_at IS NOT NULL
           OR (
               NEW.authorization_mode = 'no_charge'
               AND (NEW.actor_administrator_id IS NOT NULL OR NEW.audit_log_id IS NOT NULL)
           )
           OR (
               NEW.authorization_mode = 'administrator_no_charge'
               AND (NEW.actor_administrator_id IS NULL OR NEW.audit_log_id IS NULL)
           )
           OR NOT EXISTS (
               SELECT 1
               FROM provisioning_operations operation_row
               ...
               WHERE operation_row.id = NEW.provisioning_operation_id
                 ...
                 AND (
                     (
                         ...
                         AND preview_row.administrator_reason_code IS NULL
                         AND preview_row.administrator_reason IS NULL
                         AND NEW.actor_administrator_id IS NULL
                         AND NEW.audit_log_id IS NULL
                     )
                     OR
                     (
                         NEW.authorization_mode = 'administrator_no_charge'
                         AND preview_row.actor_administrator_id = NEW.actor_administrator_id
                         AND preview_row.administrator_reason_code IS NOT NULL
                         AND preview_row.administrator_reason IS NOT NULL
                         AND EXISTS (
                             SELECT 1
                             FROM audit_logs audit_row
                             WHERE audit_row.id = NEW.audit_log_id
                               AND audit_row.actor_type = 'administrator'
                               AND CAST(audit_row.actor_id AS UNSIGNED) = NEW.actor_administrator_id
                               AND audit_row.action = 'service.operational.reconfiguration.queued'
                               AND audit_row.target_type = 'service_subscription'
                               AND BINARY audit_row.target_id = BINARY service_row.public_id
                               AND BINARY audit_row.request_fingerprint = BINARY preview_row.request_key_hash
                               AND BINARY audit_row.correlation_id = BINARY preview_row.correlation_id
                               AND BINARY audit_row.reason_code = BINARY preview_row.administrator_reason_code
                               AND BINARY audit_row.reason = BINARY preview_row.administrator_reason
                         )
                     )
                 )
