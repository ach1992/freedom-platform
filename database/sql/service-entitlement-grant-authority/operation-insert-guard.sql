...
BEGIN
    ...
    ELSEIF COALESCE(@app_service_mutation_authority, '') = 'service_paid_mutation_queue_v1' THEN
        IF NEW.operation_type NOT IN ('renew','add_data','add_days','add_data_days','reconfigure')
        ...
        WHERE order_row.id = NEW.order_id
          ...
          AND (NEW.operation_type <> 'reconfigure'
               OR (quote_row.service_mutation_generation_snapshot = NEW.operation_generation - 1
                   AND quote_row.service_reconfiguration_preview_id IS NOT NULL
                   AND quote_row.service_target_route_selection_id_snapshot IS NOT NULL
                   AND quote_row.service_target_service_target_id_snapshot IS NOT NULL
                   AND quote_row.service_target_service_target_version_snapshot IS NOT NULL
                   AND quote_row.service_target_protocol_profile_id_snapshot IS NOT NULL
                   AND quote_row.service_target_protocol_profile_version_snapshot IS NOT NULL))
    ...
    ELSEIF COALESCE(@app_service_mutation_authority, '') = 'service_reconfiguration_no_charge_queue_v1' THEN
        ...
        END IF;
    ELSEIF COALESCE(@app_service_mutation_authority, '') = 'service_entitlement_grant_queue_v1' THEN
        IF NOT EXISTS (
           ...
           OR NEW.operation_type NOT IN ('grant_data','grant_days','grant_data_days')
           OR NEW.operation_generation < 1
           OR NEW.operation_generation <> COALESCE(@app_service_mutation_generation, 0)
           OR NEW.target_remote_identity_generation < 1
           OR NEW.request_key_hash IS NULL
           OR BINARY NEW.request_key_hash <> BINARY COALESCE(@app_service_mutation_request_hash, '')
           OR BINARY NEW.correlation_id <> BINARY COALESCE(@app_service_mutation_correlation_id, '')
           OR COALESCE(@app_service_entitlement_grant_item_id, 0) < 1
           OR NEW.state <> 'queued'
           OR NEW.state_version <> 1
           OR NEW.attempt_count <> 0
           OR NEW.effect_fence_key IS NOT NULL
           ...
           OR NEW.last_result_code IS NOT NULL
           OR NEW.last_result_message IS NOT NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Administrative Service entitlement grant operation creation authority is invalid.';
        END IF;

        SELECT service_row.id INTO valid_mutation_service_id
        FROM service_subscriptions service_row
        INNER JOIN order_items item_row ON item_row.id = service_row.order_item_id
        INNER JOIN service_entitlement_grant_items grant_item
            ON grant_item.id = COALESCE(@app_service_entitlement_grant_item_id, 0)
           AND grant_item.service_subscription_id = service_row.id
        INNER JOIN service_entitlement_grant_batches grant_batch
        ...
        WHERE service_row.id = NEW.service_subscription_id
          ...
          AND service_row.user_id = NEW.user_id
          AND service_row.mutation_generation = NEW.operation_generation
          AND grant_item.source_mutation_generation = NEW.operation_generation - 1
          AND grant_item.target_remote_identity_generation = service_row.remote_identity_generation
          AND grant_item.target_lifecycle_version = service_row.lifecycle_version
          AND grant_item.service_target_id = service_row.service_target_id
          AND grant_item.state IN ('pending','failed')
          AND grant_item.provisioning_operation_id IS NULL
          AND grant_batch.state = 'active'
          AND grant_batch.items_committed_at IS NOT NULL
          AND grant_batch.expires_at > CURRENT_TIMESTAMP(6)
          AND ((NEW.operation_type = 'grant_data' AND grant_batch.data_bytes IS NOT NULL AND grant_batch.duration_days IS NULL)
            OR (NEW.operation_type = 'grant_days' AND grant_batch.data_bytes IS NULL AND grant_batch.duration_days IS NOT NULL)
            OR (NEW.operation_type = 'grant_data_days' AND grant_batch.data_bytes IS NOT NULL AND grant_batch.duration_days IS NOT NULL))
          AND service_row.remote_deleted_at IS NULL
        ...
        IF valid_mutation_service_id IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Administrative Service entitlement grant does not match its frozen batch item and current Service authority.';
        END IF;
    ELSE
