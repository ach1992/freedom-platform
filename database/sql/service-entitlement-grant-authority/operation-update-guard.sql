...
BEGIN
    ...
    DECLARE financial_rejection_transition BOOLEAN DEFAULT FALSE;
    DECLARE paid_financial_invalidated BOOLEAN DEFAULT FALSE;
    ...
    ELSE
        IF COALESCE(@app_provisioning_authority, '') <> 'service_mutation_effect_v1'
           OR OLD.operation_type NOT IN ('reset_usage','suspend','activate','delete','rotate_subscription_link','renew','add_data','add_days','add_data_days','grant_data','grant_days','grant_data_days','reconfigure')
        ...
        IF OLD.operation_type IN ('renew','add_data','add_days','add_data_days') THEN
            SELECT EXISTS (
                SELECT 1
                FROM service_paid_mutation_authorities paid_authority
                INNER JOIN provisioning_financial_invalidations invalidation
                    ON invalidation.purchase_settlement_id = paid_authority.purchase_settlement_id
                   AND invalidation.payment_intent_id = paid_authority.payment_intent_id
                WHERE paid_authority.provisioning_operation_id = OLD.id
            ) INTO paid_financial_invalidated;
        ELSEIF OLD.operation_type = 'reconfigure' THEN
            SELECT EXISTS (
                SELECT 1
                FROM service_reconfiguration_authorities authority_row
                INNER JOIN provisioning_financial_invalidations invalidation
                    ON invalidation.purchase_settlement_id = authority_row.purchase_settlement_id
                   AND invalidation.payment_intent_id = authority_row.payment_intent_id
                WHERE authority_row.provisioning_operation_id = OLD.id
            ) INTO paid_financial_invalidated;
        END IF;
        ...
        SET financial_rejection_transition =
            COALESCE(paid_financial_invalidated, FALSE) = TRUE
            AND OLD.state IN ('queued','retry_scheduled') AND NEW.state = 'failed_final'
            AND NEW.state_version = OLD.state_version + 1
            AND NEW.attempt_count = OLD.attempt_count
            AND (NEW.effect_fence_key <=> OLD.effect_fence_key)
            AND (NEW.route_hold_expires_at <=> OLD.route_hold_expires_at)
        ...
        SET claim_transition =
            ...
            AND COALESCE(paid_financial_invalidated, FALSE) = FALSE
    ...
    IF COALESCE(claim_transition, FALSE) = FALSE
       ...
       AND COALESCE(financial_rejection_transition, FALSE) = FALSE
