...
BEGIN
    ...
    DECLARE reconfiguration_authority_count INT DEFAULT 0;
    ...
    ELSEIF COALESCE(@app_provisioning_authority, '') = 'initial_remote_effect_v1' THEN
        ...
        END IF;
    ELSEIF COALESCE(@app_service_mutation_authority, '') IN ('service_mutation_queue_v1', 'service_paid_mutation_queue_v1', 'service_reconfiguration_no_charge_queue_v1', 'service_entitlement_grant_queue_v1') THEN
        SELECT COUNT(*) INTO unresolved_mutations
        FROM provisioning_operations operation_row
        WHERE operation_row.service_subscription_id = OLD.id
    ...
    ELSEIF COALESCE(@app_provisioning_authority, '') = 'service_mutation_effect_v1' THEN
        ...
        IF COALESCE(identity_unchanged, FALSE) = FALSE
           OR (effect_operation_type <> 'reconfigure' AND COALESCE(remote_binding_unchanged, FALSE) = FALSE)
           OR (effect_operation_type <> 'reconfigure' AND NEW.remote_identity_generation <> OLD.remote_identity_generation)
           OR NEW.mutation_generation <> OLD.mutation_generation
           OR NEW.mutation_generation <> COALESCE(@app_service_mutation_generation, 0)
        ...
        ELSEIF effect_operation_type = 'delete' THEN
            ...
            END IF;
        ELSEIF effect_operation_type = 'reconfigure' THEN
            SELECT COUNT(*) INTO reconfiguration_authority_count
            FROM service_reconfiguration_authorities authority_row
            INNER JOIN plan_offering_route_selections target_selection
                ON target_selection.id = authority_row.target_route_selection_id
            INNER JOIN panel_capacity_reservations target_reservation
                ON target_reservation.id = authority_row.target_capacity_reservation_id
            INNER JOIN panel_service_targets target_row
                ON target_row.id = authority_row.target_service_target_id
            INNER JOIN panel_protocol_profiles target_profile
                ON target_profile.id = authority_row.target_protocol_profile_id
            LEFT JOIN plan_offering_route_selections source_selection
                ON source_selection.id = authority_row.source_route_selection_id
            LEFT JOIN panel_capacity_reservations source_reservation
                ON source_reservation.id = source_selection.capacity_reservation_id
            WHERE authority_row.provisioning_operation_id = (
              ...
              AND authority_row.service_subscription_id = OLD.id
              AND (authority_row.source_route_selection_id <=> OLD.route_selection_id)
              AND authority_row.source_service_target_id = OLD.service_target_id
              AND authority_row.quoted_remote_identity_generation = OLD.remote_identity_generation
              AND authority_row.quoted_lifecycle_version = OLD.lifecycle_version
              AND authority_row.quoted_mutation_generation = OLD.mutation_generation - 1
              AND authority_row.result_recorded_at IS NOT NULL
              AND authority_row.remote_result_snapshot_hash IS NOT NULL
              AND authority_row.result_remote_service_id IS NOT NULL
              AND authority_row.target_route_selection_id = NEW.route_selection_id
              AND authority_row.target_service_target_id = NEW.service_target_id
              AND BINARY authority_row.result_remote_service_id = BINARY NEW.remote_service_id
              AND target_selection.plan_offering_id = authority_row.target_plan_offering_id
              AND target_selection.selected_service_target_id = authority_row.target_service_target_id
              AND target_selection.panel_protocol_profile_id = authority_row.target_protocol_profile_id
              AND target_selection.capacity_reservation_id = authority_row.target_capacity_reservation_id
              AND target_reservation.state = 'committed'
              AND target_reservation.units = 1
              AND target_row.state = 'active'
              AND target_row.capability_status = 'verified'
              AND target_row.version = authority_row.target_service_target_version
              AND target_profile.state = 'active'
              AND target_profile.version = authority_row.target_protocol_profile_version
              AND (authority_row.source_route_selection_id IS NULL
                   OR (source_reservation.state = 'released' AND source_reservation.units = 1));

            IF reconfiguration_authority_count <> 1
               OR OLD.lifecycle_state NOT IN ('active','suspended')
               OR NEW.lifecycle_state <> OLD.lifecycle_state
               OR NEW.lifecycle_version <> OLD.lifecycle_version + 1
               OR NEW.remote_identity_generation <> OLD.remote_identity_generation + 1
               OR NEW.mutation_generation <> OLD.mutation_generation
               ...
               OR OLD.remote_deleted_at IS NOT NULL
               OR NEW.remote_deleted_at IS NOT NULL THEN
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Service reconfiguration transition is not allowed.';
            END IF;
        ELSE
