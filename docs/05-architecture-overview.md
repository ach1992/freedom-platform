...
Critical rollback must also preserve protection across dependency-sensitive DDL. For the Telegram delivery authority, an authority table keeps its own guards until the table itself is successfully dropped; `telegram_delivery_operations` is dropped before the capability table, and shared Outbox guards are removed only after both authority tables are gone. If an independent privileged DDL actor introduces an incoming FK after final attestation, the affected table `DROP` fails while every surviving authority table remains guarded. A capability-only partial rollback is explicitly attested and resumable after the external dependency is removed; unrecognized partial surfaces fail closed. Cooperative advisory locks may serialize migration runners, but they are not treated as proof against unrelated DDL.
