...
Every migration-created durable table has one exact feature owner or reviewed infrastructure classification in `scripts/ci/architecture-boundaries.php`; CI verifies that the map is complete and not stale, including helper-created `Schema::create(...)` tables only when their literal callsites can be proven. Unresolved or non-literal creation fails closed. The same owner map rejects cross-owner mutations. Runtime persistence must remain statically attributable: literal table chains and statically bounded literal table sets are allowed when ownership matches, while unbounded dynamic mutations, deferred mutable Query Builder handles, opaque raw DML/DDL, and Eloquent persistence under `app/Modules` or `app/Shared` fail closed. Literal `SET` statements that only establish connection-scoped MariaDB session authority are non-durable and allowed. The historical Agent pricing trigger helper is the sole exact migration-only raw trigger-DDL helper and remains tracked under #183; new runtime raw DDL is not permitted by that exception. Cross-cutting `audit_logs` is a shared append-only sink: Application code may append records, but update/delete-style mutations remain forbidden. Pre-existing cross-owner runtime seams may be grandfathered only by exact `source-path|table` entries tracked for removal in #188; wildcards are not accepted and stale/unused exceptions fail CI.
