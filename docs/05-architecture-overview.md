...
Durable asynchronous contracts are versioned independently from their event type. Every new Outbox row carries an explicit positive `contract_version`; rows persisted before this invariant deterministically read as version `1`. Dispatch routing uses the pair `(event_type, contract_version)`, so an evolved handler can coexist with the prior version until the older durable backlog is drained. Unknown versions fail before a domain handler can perform an external effect. Idempotent replay compares the stored contract version as part of the immutable envelope. Before removing an old handler version, operators must run `operations:check-outbox-contract-retirement <event-type> <version>` and receive a safe result; schema rollback also refuses to erase versioning after non-v1 durable history exists. Payload hashing and secret-free Outbox rules remain unchanged.

Contract evolution within one `contract_version` is backward-compatible only: every durable payload ever emitted under that version must remain valid under the current handler with the same field meaning, idempotency identity and external-effect authority. An optional additive field may remain on the same version only when older payloads that omit it retain identical semantics through an explicit backward-compatible default. Removing or renaming a field, adding a required field, changing a field type/domain/default/meaning, changing aggregate or idempotency identity, changing external-effect meaning/authority, or otherwise interpreting a previously persisted payload differently requires a new `contract_version`. A release that introduces a new version must keep handlers for every older version that can still have unprocessed durable rows; an old handler may be removed only after the retirement guard proves that exact `(event_type, contract_version)` is drained.
