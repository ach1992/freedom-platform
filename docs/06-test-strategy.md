...
- **FULL** — the fail-safe fallback for existing-module product changes, modified/deleted migrations, global/config/runtime/dependency surfaces, ambiguous relationships, missing Feature evidence, or anything the selector does not explicitly understand. The complete Feature suite runs against MariaDB 10.11 + authenticated Redis. For normal PR acceptance, the complete file set may be partitioned deterministically across sixteen isolated GitHub-hosted runners, each with its own disposable MariaDB/Redis stack; the union must equal the complete Feature suite exactly once and the aggregate integration gate succeeds only when every shard succeeds. Sharding changes wall-clock placement, not coverage or acceptance semantics.
...
Normal PR and filtered diagnostic integration fail fast on the first PHPUnit error/failure. This changes only failed-run latency and diagnostics: a green TARGETED or FULL run still executes every test in its selected scope. A normal PR FULL run may use sixteen isolated shards as described above; a TARGETED run uses only the selected tests, and an unfiltered manual FULL validation intentionally remains a single complete PHPUnit + coverage path as the explicit regression backstop so coverage output never requires cross-shard merging.
...
bash scripts/ci/test-feature-sharding.sh
