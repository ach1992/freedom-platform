...
| **Apply** | Run the ordinary timestamped migration. While convergence is active, `nowpayments_terminal_conflict_upgrade_fence` plus migration-owned triggers deliberately reject new purchase/payment authority/effect writes. The migration composes canonical guards with `CREATE OR REPLACE TRIGGER`, revalidates durable conflict evidence under the fence, normalizes exact legacy conflicts, verifies the resulting fail-closed state, and only then removes the temporary fence. | Do not delete/disable the fence table or temporary triggers to restore checkout availability, and do not edit reconciliation findings/observations to force migration progress. |
