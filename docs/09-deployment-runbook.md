...
The NOWPayments terminal-conflict hardening migration [`2026_09_21_000100_harden_nowpayments_terminal_provider_conflict.php`](../database/migrations/2026_09_21_000100_harden_nowpayments_terminal_provider_conflict.php) is a financial-authority migration. Apply it only from the exact reviewed release revision after the normal release gate and backup requirements above have passed. Quiescing purchase/payment workers is still required operational hygiene, but it is not the mechanical race barrier: the migration owns a persistent database fence before its first legacy-conflict decision read.

| Release stage | Required operator evidence and action | Unsafe shortcut prohibited by the contract |
|---|---|---|
| **Preflight** | Confirm exact-revision CI is green, including MariaDB integration coverage; take the reviewed backup; quiesce purchase/payment workers and entrypoints; confirm no older application revision can still issue provider mutations; confirm the current schema is the expected predecessor rather than an unknown partial/manual state. | Do not rely on quiescence alone, manually normalize terminal payment rows, or infer safety from a provider/dashboard view. |
| **Apply** | Run the ordinary timestamped migration from the exact reviewed application revision. It first persists `nowpayments_terminal_conflict_upgrade_fence`, then acquires every slot in the bounded purchase-provider mutation pool before the first legacy-conflict read. Current runtime provider value mutations hold one pool slot from their final local preparation through durable provider evidence/convergence, and reject before the external call whenever the persistent fence is active. After in-flight provider effects drain, migration-owned triggers reject new purchase/payment authority/effect writes; canonical guards are composed with `CREATE OR REPLACE TRIGGER`, durable conflict evidence is revalidated and normalized, and the temporary fence is removed only while the entire provider-mutation pool remains exclusively held. | Do not delete/disable the fence table or temporary triggers to restore checkout availability, bypass `PurchaseProviderMutationBarrier`, run an older application revision against the fenced schema, or edit reconciliation findings/observations to force migration progress. |
| **Postflight** | Verify the migration is recorded; `nowpayments_terminal_conflict_upgrade_fence` and every `*_np_conflict_upgrade_*_fence` trigger are absent; canonical `payment_intents_insert_guard`, `payment_intents_update_guard`, and `nowpayments_authority_update_guard` exist; run the reviewed health/smoke path before reopening workers. | Do not reopen purchase/payment traffic merely because the migration command returned; absence of the temporary fence and presence of the canonical guard set are part of acceptance. |
| **Interrupted or failed apply** | Treat continued purchase rejection as intentional fail-closed containment. Preserve diagnostics and the database, keep workers quiesced, establish the exact schema/fence state, then re-run the same migration from the same reviewed release revision so its idempotent fence/composition path can converge. Escalate to the approved restore/forward-fix decision if the state cannot be proven. | Never manually drop the persistent fence/table/triggers, bypass canonical guards, or switch to an older application revision merely to make payment traffic appear healthy. |

Rollback is exceptional. The migration activates the same persistent exclusion boundary before restoring predecessor guards and refuses semantic rollback when terminal-conflict reconciliation evidence exists. A refusal means retain the current schema and use an approved forward fix or backup/restore decision; do not delete evidence or weaken the fence to make `migrate:rollback` succeed.
