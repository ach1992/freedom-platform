...
3. Use the current repository operating rules and live GitHub delivery state for task persistence, PR/review flow, and validation/integration mechanics; do not restate those mechanics in this product specification. Dynamic priority, status, blockers, decisions, branches, SHAs, reviews, and CI results do not belong in repository status files or Chat.
