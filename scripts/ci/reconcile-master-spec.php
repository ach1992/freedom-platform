...
$path = 'docs/specification/master-execution-prompt.md';
$text = file_get_contents($path);
if ($text === false) {
    fwrite(STDERR, "Unable to read master specification.\n");
    exit(1);
}
...
$newAuthority = "**Authoritative rule:** This document is the normative Version 1 product, security, correctness, runtime, integration, testing, deployment, and final-acceptance specification. Implement its capabilities, invariants, and delivery boundaries; do not invent business integrations or silently omit a defined requirement. Repository execution mechanics and live delivery state are governed by `AGENTS.md`, `CONTRIBUTING.md`, the canonical engineering references, and live GitHub.\n**Repository governance reconciliation:** `2026-08-11` — process-only instructions that required duplicate status, traceability, handoff, or per-task evidence artifacts are superseded; Version 1 product/security/correctness scope is unchanged.\n";
if (substr_count($text, $oldAuthority) !== 1) {
    fwrite(STDERR, "Expected authoritative-rule baseline was not found exactly once.\n");
    exit(1);
}
$text = str_replace($oldAuthority, $newAuthority, $text);

$section0Start = strpos($text, "# 0. Primary Command to the Lead Agent\n");
$section1Marker = "\n---\n\n# 1. Project Mission\n";
$section1Start = $section0Start === false ? false : strpos($text, $section1Marker, $section0Start);
if ($section0Start === false || $section1Start === false) {
    fwrite(STDERR, "Unable to locate Section 0 boundaries.\n");
    exit(1);
}
...
MD;
$text = substr($text, 0, $section0Start).$newSection0.substr($text, $section1Start);

$workflowStart = strpos($text, "## 3.11 Team workflow rules\n");
$section4Marker = "\n---\n\n# 4. Architecture\n";
$section4Start = $workflowStart === false ? false : strpos($text, $section4Marker, $workflowStart);
if ($workflowStart === false || $section4Start === false) {
    fwrite(STDERR, "Unable to locate Section 3.11 boundaries.\n");
    exit(1);
}
...
MD;
$text = substr($text, 0, $workflowStart).$newWorkflow.substr($text, $section4Start);

if (file_put_contents($path, $text) === false) {
    fwrite(STDERR, "Unable to write master specification.\n");
    exit(1);
}
