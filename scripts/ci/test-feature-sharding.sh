...
find "$root/tests/Feature" -type f -name '*.php' -printf '%P\n' | sed 's#^#tests/Feature/#' | sort > "$all"
...
((max_weight * 100 <= min_weight * 115))     || fail "2-way shard static weights are imbalanced: $weight_zero vs $weight_two"
