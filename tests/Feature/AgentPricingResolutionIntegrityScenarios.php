...
trait AgentPricingResolutionIntegrityScenarios
{
    ...
    public function test_mariadb_enforces_foreign_keys_uniqueness_checks_hashes_and_immutability(): void
    {
        ...
        $stored = DB::table('agent_pricing_profile_versions')->where('id', $profile->versionId)->first();
        self::assertNotNull($stored);
        $invalid = (array) $stored;
        unset($invalid['id']);
        $invalid['mutation_key'] = 'agt.db.invalid.check';
        ...
        $invalid['mutation_key'] = 'agt.db.invalid.hash';
        $invalid['state'] = 'active';
        $invalid['configuration_hash'] = str_repeat('0', 64);
        $this->assertQueryRejected(static fn (): bool => DB::table('agent_pricing_profile_versions')->insert($invalid));
        $storedRule = DB::table('agent_pricing_rule_versions')->where('id', $rule->versionId)->first();
        self::assertNotNull($storedRule);
        $invalidRule = (array) $storedRule;
        unset($invalidRule['id']);
        $invalidRule['mutation_key'] = 'agt.db.invalid.amount';
        $invalidRule['mutation_payload_hash'] = hash('sha256', 'invalid-amount');
        $invalidRule['version'] = 2;
        $invalidRule['override_price_irr'] = -1;
        $this->assertQueryRejected(static fn (): bool => DB::table('agent_pricing_rule_versions')->insert($invalidRule));
        $storedResolution = DB::table('agent_pricing_resolutions')->where('id', $resolution->resolutionId)->first();
        ...
        $invalidResolution['resolution_key'] = 'agt.resolve.db.invalid-hash';
