...
{
    private function createProfile(AgentPricingService $service, int $administratorId, string $profileCode, bool $discountCombinationAllowed): \App\Modules\Agents\Application\AgentPricingProfileVersionReceipt
    {
    ...
    private function assertRuntimeMessage(string $message, callable $callback): void { try { $callback(); self::fail('Expected RuntimeException was not thrown.'); } catch (RuntimeException $exception) { self::assertSame($message, $exception->getMessage()); } }
    private function assertDomainMessage(string $message, callable $callback): void { try { $callback(); self::fail('Expected DomainException was not thrown.'); } catch (DomainException $exception) { self::assertSame($message, $exception->getMessage()); } }
    private function assertAuthorizationDenied(callable $callback): void { try { $callback(); self::fail('Expected authorization denial was not thrown.'); } catch (AuthorizationException) { self::assertTrue(true); } }
    private function assertInvalidArgument(callable $callback): void { try { $callback(); self::fail('Expected InvalidArgumentException was not thrown.'); } catch (InvalidArgumentException) { self::assertTrue(true); } }
    private function assertQueryRejected(callable $callback): void { try { $callback(); self::fail('Expected MariaDB query rejection was not thrown.'); } catch (QueryException) { self::assertTrue(true); } }
}
