...
final class TelegramCardToCardReceiptStatusLocalizationResolverTest extends TestCase
{
    ...
    public function test_status_delivery_uses_canonical_resolver_and_observes_requested_locale_override(): void
    {
        ...
        self::assertSame(
            trans('telegram_c2c_receipt.received', [], 'fa'),
            $translation->invoke($delivery, 'telegram_c2c_receipt.received', 'fa'),
        );
        self::assertSame(
            trans('telegram_c2c_receipt.received', [], 'en'),
            $translation->invoke($delivery, 'telegram_c2c_receipt.received', 'en'),
        );

        $administratorId = $this->administrator();
        $now = now('UTC');
        DB::table('localization_overrides')->insert([
            'translation_key' => 'telegram_c2c_receipt.received',
            'locale' => 'fa',
