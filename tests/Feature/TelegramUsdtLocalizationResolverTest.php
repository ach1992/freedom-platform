...
final class TelegramUsdtLocalizationResolverTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_usdt_handler_uses_canonical_resolver_and_observes_placeholder_override(): void
    {
        $handler = $this->app->make(TelegramUsdtNavigationHandler::class);
        ...
        $key = 'telegram.navigation.purchase.payment_methods.usdt_payment.instructions';
        $replace = [
            'network' => 'BEP20',
            'amount' => '12.345678',
            'address' => 'wallet-address-test',
            'expires_at' => '14:30',
        ];
        ...
        DB::table('localization_overrides')->insert([
            'translation_key' => $key,
            'locale' => 'fa',
            'override_value' => 'شبکه :network | مبلغ :amount | مقصد :address | اعتبار :expires_at',
            'version' => 1,
        ...
        self::assertSame(
            'شبکه BEP20 | مبلغ 12.345678 | مقصد wallet-address-test | اعتبار 14:30',
            $translation->invoke($handler, $key, 'fa', $replace),
        );
        self::assertSame(trans($key, $replace, 'en'), $translation->invoke($handler, $key, 'en', $replace));
    }

    public function test_usdt_handler_rejects_missing_localization_instead_of_emitting_sentinel(): void
    {
        $handler = $this->app->make(TelegramUsdtNavigationHandler::class);
        $translation = new ReflectionMethod($handler, 'translation');

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Telegram USDT translation is unavailable.');

        $translation->invoke($handler, 'telegram.navigation.purchase.payment_methods.usdt_payment.missing_copy', 'fa', []);
    }

    public function test_usdt_handler_fails_closed_when_required_replacements_are_missing(): void
    {
        $handler = $this->app->make(TelegramUsdtNavigationHandler::class);
        $translation = new ReflectionMethod($handler, 'translation');
        ...
        $translation->invoke(
            $handler,
            'telegram.navigation.purchase.payment_methods.usdt_payment.instructions',
            'fa',
            ['network' => 'BEP20'],
        );
    }
