...
final class TelegramZarinpalLocalizationResolverTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_zarinpal_namespace_override_is_observed_and_english_remains_isolated(): void
    {
        $handler = $this->app->make(TelegramZarinpalNavigationHandler::class);
        $translation = new ReflectionMethod($handler, 'translation');
        $key = 'telegram_zarinpal.redirect';
        ...
        DB::table('localization_overrides')->insert([
            'translation_key' => $key,
            'locale' => 'fa',
            'override_value' => 'درگاه آزمایشی زرین‌پال آماده است.',
            'version' => 1,
        ...
        self::assertSame('درگاه آزمایشی زرین‌پال آماده است.', $translation->invoke($handler, $key, 'fa', []));
        self::assertSame(trans($key, [], 'en'), $translation->invoke($handler, $key, 'en', []));
    }

    public function test_zarinpal_handler_uses_canonical_resolver_and_observes_placeholder_override(): void
    {
        $handler = $this->app->make(TelegramZarinpalNavigationHandler::class);
        ...
        DB::table('localization_overrides')->insert([
            'translation_key' => $key,
            'locale' => 'fa',
            'override_value' => 'شماره :number | روش :method',
            'version' => 1,
            'updated_by_administrator_id' => $administratorId,
            'created_at' => $now,
            'updated_at' => $now,
        ...
        self::assertSame(
            'شماره 1 | روش Zarinpal',
            $translation->invoke($handler, $key, 'fa', $replace),
        );
        self::assertSame(trans($key, $replace, 'en'), $translation->invoke($handler, $key, 'en', $replace));
    }

    public function test_zarinpal_handler_rejects_missing_localization_instead_of_emitting_sentinel(): void
    {
        $handler = $this->app->make(TelegramZarinpalNavigationHandler::class);
        $translation = new ReflectionMethod($handler, 'translation');

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Telegram Zarinpal translation is unavailable.');

        $translation->invoke($handler, 'telegram_zarinpal.missing_copy', 'fa', []);
    }

    public function test_zarinpal_handler_fails_closed_when_required_replacements_are_missing(): void
    {
        $handler = $this->app->make(TelegramZarinpalNavigationHandler::class);
        $translation = new ReflectionMethod($handler, 'translation');
        ...
        $translation->invoke(
            $handler,
            'telegram.navigation.purchase.payment_methods.item',
            'fa',
            ['number' => '1'],
        );
    }
